...
public:
	...
	result RequestImageUpload(Tizen::Net::Http::IHttpTransactionEventListener* listener, Tizen::Base::String url,  Tizen::Base::ByteBuffer* pBuffer);	// [file], filename(formatting -> pkeykichul_20140417153111.png)	// 갤러리 -> 파일 선택 -> 그 파일 경로를 가져오고, filename을 설정해서 서버로 전송

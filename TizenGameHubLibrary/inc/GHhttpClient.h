...
public:
	...
	result RequestImageUpload();	// [file], filename(formatting -> pkeykichul_20140417153111.png)	// 갤러리 -> 파일 선택 -> 그 파일 경로를 가져오고, filename을 설정해서 서버로 전송
	result RequestImageDownload();	// url+[filename] ->download    ==>  byte array -> file로 만들고 <device 폴더>를 만들고 거기에 저장	"pkeykichul_20140417153111.png"

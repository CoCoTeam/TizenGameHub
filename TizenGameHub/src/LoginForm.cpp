...
LoginForm::OnActionPerformed(const Tizen::Ui::Control& source, int actionId)
{
	...
	case IDA_BUTTON_LOGIN:
		//!! do Login
		doLogin();
		if(isLogin == true)	// (로그인 성공 시) 로그인, 개인페이지로 이동
		{
		...
		{
			msgBox.Construct(L"Login Fail", L"LOGIN", MSGBOX_STYLE_OK);
...
void LoginForm::OnTransactionAborted(HttpSession& httpSession, HttpTransaction& httpTransaction, result r)
{
...
void LoginForm::OnTransactionReadyToWrite(HttpSession& httpSession, HttpTransaction& httpTransaction, int recommendedChunkSize)
{
	AppLog("OnTransactionReadyToWrite");
}

void LoginForm::OnTransactionHeaderCompleted(HttpSession& httpSession, HttpTransaction& httpTransaction, int headerLen, bool authRequired)
{
	AppLog("OnTransactionHeaderCompleted");
}

void LoginForm::OnTransactionCompleted(HttpSession& httpSession, HttpTransaction& httpTransaction)
{
...
void LoginForm::OnTransactionCertVerificationRequiredN(HttpSession& httpSession, HttpTransaction& httpTransaction, Tizen::Base::String* pCert)
{

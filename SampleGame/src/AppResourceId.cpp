#include "AppResourceId.h"

const wchar_t* GameMain = L"GameMain";
const wchar_t* GameMain2 = L"GameMain2";
const wchar_t* GameSelect = L"GameSelect";
const wchar_t* IDC_BUTTON_ACHIEVEMENT = L"IDC_BUTTON_ACHIEVEMENT";
const wchar_t* IDC_BUTTON_LANK = L"IDC_BUTTON_LANK";
const wchar_t* IDC_BUTTON_LEADERBOARD = L"IDC_BUTTON_LEADERBOARD";
const wchar_t* IDC_BUTTON_MODE1 = L"IDC_BUTTON_MODE1";
const wchar_t* IDC_BUTTON_MODE2 = L"IDC_BUTTON_MODE2";
const wchar_t* IDC_LISTVIEW1 = L"IDC_LISTVIEW1";
const wchar_t* IDC_TEXTBOX_MESSAGE = L"IDC_TEXTBOX_MESSAGE";
const wchar_t* RANKING_LISTVIEW1 = L"RANKING_LISTVIEW1";
const wchar_t* Ranking = L"Ranking";
const wchar_t* IDC_BUTTON1 = L"IDC_BUTTON1";
const wchar_t* IDC_BUTTON2 = L"IDC_BUTTON2";
const wchar_t* IDC_BUTTON3 = L"IDC_BUTTON3";
const wchar_t* IDC_BUTTON4 = L"IDC_BUTTON4";
const wchar_t* IDC_BUTTON5 = L"IDC_BUTTON5";
const wchar_t* IDC_BUTTON6 = L"IDC_BUTTON6";
const wchar_t* IDC_BUTTON7 = L"IDC_BUTTON7";
const wchar_t* IDC_BUTTON8 = L"IDC_BUTTON8";
const wchar_t* IDC_BUTTON9 = L"IDC_BUTTON9";
const wchar_t* IDC_BUTTON_OK = L"IDC_BUTTON_OK";
const wchar_t* IDC_LABEL1 = L"IDC_LABEL1";
const wchar_t* IDC_LABEL2 = L"IDC_LABEL2";
/*const wchar_t* IDC_LISTVIEW1 = L"IDC_LISTVIEW1";*/
const wchar_t* IDF_POPUP = L"IDF_POPUP";


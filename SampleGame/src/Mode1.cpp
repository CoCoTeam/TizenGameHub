...
void
Mode1::OnActionPerformed(const Tizen::Ui::Control& source, int actionId)
{
	...
	case ID_BUTTON_OK:
		{
			...
			String str;
			pref.get(pref.name, str);
//			pref.get(pref.name,"ZZZZ");
			//pref.get(PrefClass::name,"ZZZZ");
...
void
Mode1::OnUserEventReceivedN (RequestId requestId, Tizen::Base::Collection::IList *pArgs)
...
void Mode1::ShowPopup(Popup* __pPopup) {
	__pPopup->SetShowState(true);
	__pPopup->Show();
}

void Mode1::HidePopup(Popup* __pPopup) {
...
// timer 작동
void
Mode1::OnTimerExpired(Timer&  timer)
{
	SetTimer();
...
void
Mode1::OnFormBackRequested(Tizen::Ui::Controls::Form& source)
{
//	AppLog("--------------------8-----------------");
//

...
void
Mode2::OnActionPerformed(const Tizen::Ui::Control& source, int actionId)
{
...
void
Mode2::OnUserEventReceivedN (RequestId requestId, Tizen::Base::Collection::IList *pArgs)
//Form1::OnUserEventReceivedN (RequestId requestId,int pArgs)
{
	...
	if(requestId == 100)
	{
		...
		//pButton[btn_num]->SetColor(ButtonStatus(2),Color::GetColor(COLOR_ID_RED));
...
void
Mode2::InsertItem(void)
{
	//__pWatchTextList = new (std::nothrow) ArrayList();
	String __getWatch = __pLabel->GetText();  //__watchText 를 얻어옴
...
void Mode2::ShowPopup(Popup* __pPopup) {
	__pPopup->SetShowState(true);
	__pPopup->Show();
}

void Mode2::HidePopup(Popup* __pPopup) {
...
// timer 작동
void
Mode2::OnTimerExpired(Timer&  timer)
{
	SetTimer();

	result r = __pTimer->Start(10);

	if(__pLabel2->GetText() == "00:10:00")
	{
		__pTimer->Cancel();
		pThread->setlive(false);
		__pButtonOk->SetEnabled(true);
		__pButtonLank->SetEnabled(true);
...
void
Mode2::OnFormBackRequested(Tizen::Ui::Controls::Form& source)
{
	SceneManager* pSceneManager = SceneManager::GetInstance();
	AppAssert(pSceneManager);
